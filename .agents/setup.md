...
6. Wire each working local command into ignored `threedee.local.toml` or another untracked config passed with `--config`.
...
2. Install TRELLIS.2 and get its example image-to-GLB path working manually.
3. Create a small wrapper that accepts `--input`, `--output`, and `--seed`.
4. Start with the fastest supported pipeline/resolution to prove the CLI contract.
5. Wire the wrapper into local config:
   ```toml
